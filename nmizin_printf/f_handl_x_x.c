...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/04/15 13:08:57 by nmizin            #+#    #+#             */
/*   Updated: 2018/04/15 13:11:22 by nmizin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		f_part_two(t_var *v, char *argv)
{
	...
	if (v->w < 0)
	{
		if ((v->min) && (v->min = 0))
			;
		v->w = (v->w * -1) <= v->l ? 0 : (v->w * -1) -
			(v->l + v->spa + v->p + v->min + v->pl + v->hes);
	}
	if (*argv != '0' && !v->zer)
		v->bp += write(1, v->res == 'x' ? "0x" : "0X", v->hes);
	v->bp += f_w_e_l('0', v->p);
...
static int		f_part_one(t_var *v, char *argv)
{
	...
	if (!v->zer)
		...
		v->p = v->p - v->l - v->spa - v->min - v->pl - v->hes < 0 ?
	0 : v->p - v->l - v->spa - v->min - v->pl - v->hes;
	if (v->w >= 0)
		v->w <= v->l ? (v->w = 0) :
	(v->w -= (v->l + v->spa + v->p + v->min + v->pl + v->hes));
	f_part_two(v, argv);
	return (1);
}
